...
class Log(object):
    ...
    def __init__(self):
        ...
        self.logname = os.path.join(log_dir, '%s.log' % time.strftime('%Y-%m-%d-%H-%I-%M'))
        # 生成记录器对象
        self.logger = logging.getLogger()
        # 设置日志级别
        self.logger.setLevel(logging.DEBUG)
        # 日志输出格式
        fm = '[%(asctime)s]|%(levelname)-6s: %(message)s'
...
if __name__ == '__main__':
    ...
    print(log_dir)
    cur_path = os.path.abspath(os.path.dirname(os.getcwd()))+"\logdata"
    print("path:"+cur_path)